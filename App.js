...
export default class App extends Component {
  ...
  onEnterText = (input, inputtype) => {
    // to use regEx in validation
    let alpha = /^[a-zA-Z]+$/
    let number = /^[0-9]+$/
    
    // validate `username` field
    if (inputtype == 'username'){
      if (input.length <= 3 || !alpha.test(input)){
    ...
    if (inputtype == 'password'){
      if (input.length <= 8 || !number.test(input)){
      ...
      <View style = {styles.container}>
        <TextInput 
          style = { [styles.TextInputStyle,
            this.state.error_username ? styles.errorBorder : null
          ]}
        ...
        />
        {
          this.state.error_username == true ? (
            <Text style = {styles.errorMessage}>
              * Use only letters and minimum 3 symbols!
            </Text>
          ) : null
        }
        <TextInput 
          style = { [styles.TextInputStyle,
            this.state.error_password ? styles.errorBorder : null
          ]}
        ...
        />
        {
          this.state.error_password == true ? (
            <Text style = {styles.errorMessage}>
              * Use only digits and minimum 8 symbols!
            </Text>
          ) : null
        }
        <Button 
